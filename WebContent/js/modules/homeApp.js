var homeApp = angular.module('homeApp',
    ['categoriesAndSubApp', 'constantsApp']);
...
homeApp.controller('HomeController', [
    ...
    'enabled_disabled',
    'itemCollection',
    function($scope, $http, $location, $window, categories, filterFilter,
        enabled_disabled, itemCollection) {
        ...
        $http.get("json/portfolioItems.json").then(
            function(response) {
                homeCtrl.portfolioItems = filterFilter(
                    response.data.PortfolioItems,
                    itemCollection.New);
            });

        // Recupera la lista de categorias disponibles
        $http.get("json/homeCategories.json").then(
            function(response) {
                homeCtrl.homeCategories = filterFilter(
                    response.data.HomeCategories,
                    enabled_disabled.Enabled);
            });

        // Ir a comprar
        homeCtrl.goToPurchase = function(itemSelected) {
            sessionStorage.setItem('itemSelectedToPurchase', JSON
                .stringify(itemSelected));
            $window.location.href = location.origin
                + '/OnlineStore/html/purchase.html';
        }
        ...
        homeCtrl.sendMessage = function() {
            var req = {
                method: 'POST',
                url: location.origin
                + "/OnlineStore/rest/SendEmailRest/sendEmail",
                headers: {
        ...
        homeCtrl.redirectTo = function(url) {
            $window.location.href = location.origin + '/OnlineStore/html/'
                + url + '.html';
        }
        homeCtrl.redirectToPoliticaEnvio = function() {
            $window.location.href = location.origin + '/OnlineStore/html/politicasEnvio.html';
        }

        homeCtrl.redirectToFormaPago = function() {
            $window.location.href = location.origin + '/OnlineStore/html/formaPago.html';
        }
        // Ejecucion inicial de la aplicacion
        init();
        function init() {
            $http.get(
                location.origin + "/OnlineStore/rest/InitApp/writeLog")
                .then(
                function(response) {
                    sessionStorage.setItem("ipAddress",
                        response.data.ip.hostAddress);
                });
